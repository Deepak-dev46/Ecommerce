...
/* ══════════════════════════════════════════════════════════════════
   WORKLOAD CHART  (US-149)
══════════════════════════════════════════════════════════════════ */
function WorkloadChart({ personnel, chartError, accentColor }) {
  const AMBER = '#F59E0B';
  const BRAND_C = '#27235C';

  /* Empty state */
  if (!chartError && personnel.length === 0) {
    return (
      <Paper elevation={0} sx={{
        border: '1px solid #E5E7EB', borderRadius: '12px',
        p: 3, mb: 2.5, textAlign: 'center', color: '#9CA3AF',
      }}>
        <PersonIcon sx={{ fontSize: 36, opacity: 0.3, mb: 1 }} />
        <Typography sx={{ fontSize: '0.85rem', fontWeight: 500 }}>
          No agents available — workload chart cannot be displayed
        </Typography>
      </Paper>
    );
  }

  /* Fallback: chart load error → plain text list */
  if (chartError) {
    return (
      <Paper elevation={0} sx={{
        border: '1px solid #FEE2E2', borderRadius: '12px',
        p: 2.5, mb: 2.5, bgcolor: '#FFF5F5',
      }}>
        <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: '#991B1B', mb: 1.5 }}>
          Chart unavailable — showing text summary
        </Typography>
        <Stack spacing={0.8}>
          {personnel.map(p => (
            <Stack key={p.supportPersonId} direction="row" justifyContent="space-between"
              alignItems="center">
              <Typography sx={{ fontSize: '0.82rem', color: '#374151' }}>
                {p.supportPersonName}
              </Typography>
              <LoadChip count={p.assignedTicketCount ?? 0} />
            </Stack>
          ))}
        </Stack>
      </Paper>
  ...
  /* Find max load for amber highlight */
  const maxCount = Math.max(...personnel.map(p => p.assignedTicketCount ?? 0));

  const data = personnel.map(p => ({
    name: p.supportPersonName?.split(' ')[0] ?? `#${p.supportPersonId}`,
    count: p.assignedTicketCount ?? 0,
    full: p.supportPersonName,
  }));
  ...
  return (
    <Paper elevation={0} sx={{
      ...
      <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1.5 }}>
        <Box sx={{
          width: 22, height: 22, borderRadius: '6px', bgcolor: BRAND_C,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <AssignmentIndIcon sx={{ fontSize: 13, color: '#fff' }} />
        </Box>
        <Typography sx={{
        ...
        <Chip label="Live" size="small" sx={{
          ...
          bgcolor: '#EDFAF2', color: '#166534', borderRadius: '4px',
        }} />
        <Box sx={{ flex: 1 }} />
        <Stack direction="row" alignItems="center" spacing={0.5}>
          <Box sx={{ width: 10, height: 10, borderRadius: '2px', bgcolor: AMBER }} />
          <Typography sx={{ fontSize: '0.68rem', color: '#6B7280' }}>Highest load</Typography>
          <Box sx={{ width: 10, height: 10, borderRadius: '2px', bgcolor: accentColor, ml: 1 }} />
          <Typography sx={{ fontSize: '0.68rem', color: '#6B7280' }}>Others</Typography>
        </Stack>
      </Stack>
      ...
      <ResponsiveContainer width="100%" height={160}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
          ...
          <ReTooltip
            formatter={(val, _, { payload }) => [`${val} active tickets`, payload?.full ?? '']}
          ...
          <Bar dataKey="count" radius={[4, 4, 0, 0]}>
            {data.map((entry, i) => (
              <Cell
                key={i}
                fill={entry.count === maxCount && maxCount > 0 ? AMBER : accentColor}
                fillOpacity={0.85}
              />
            ))}
          </Bar>
...
function AssignPanel({ type }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // let capList = [];
      // try {
      //   // const capRaw = await fetchJson(`${ASSIGN_URL}/api/assignments/workload-summary`);
      //   // capList = Array.isArray(capRaw) ? capRaw : Array.isArray(capRaw?.data) ? capRaw.data : [];
      //   // setChartError(false);
      //   const capRaw = await fetchJson(`${ASSIGN_URL}/api/assignments/workload-summary`);
      //   const capList = Array.isArray(capRaw)
      //     ? capRaw
      ...
      // } catch {
      //   // Chart fails → try legacy endpoint so the table still works
      //   try {
      //     // const capRaw = await fetchJson(`${ASSIGN_URL}/api/assignments/capacity/all`);
      //     // capList = Array.isArray(capRaw) ? capRaw : Array.isArray(capRaw?.data) ? capRaw.data : [];
      //     const capRaw = await fetchJson(`${ASSIGN_URL}/api/assignments/workload-summary`);
  ...
  const doAssign = async () => {
    ...
    try {
      ...
      // }
        ...
        // Write to ticket_assignments so workload chart reflects manual assignment
        // supportPersonId set → triggerAssignment treats it as manual → status=OPEN, no acknowledgement
        try {
          await fetchJson(`${ASSIGN_URL}/api/assignments/trigger`, {
            method: 'POST',
            body: JSON.stringify({
              ticketId: tid,
              supportPersonId: personId,
              priority: selItem.priority || 'MEDIUM',
              assignedByName: 'ITSM Manager',
            }),
          });
        } catch (e) {
          // Non-fatal — ticket is assigned, workload count may lag
          console.warn('Could not write assignment record:', e.message);
        }
      }
    ...
    <Box>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems="flex-start">
        ...
        <Paper elevation={0} sx={{ flex: 1, border: '1px solid #E5E7EB', borderRadius: '12px', overflow: 'hidden', minWidth: 0 }}>
          {/* Header */}
          <Box sx={{
            px: 2.5, py: 1.6, borderBottom: '1px solid #F3F4F6', bgcolor: '#FAFAFA',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between'
          }}>
            <Stack direction="row" alignItems="center" spacing={1}>
              <Box sx={{
                width: 22, height: 22, borderRadius: '6px', bgcolor: accentColor,
                display: 'flex', alignItems: 'center', justifyContent: 'center'
              }}>
            ...
            <Button size="small" onClick={load}
              ...
              sx={{
                fontSize: '0.72rem', fontWeight: 600, color: accentColor,
          ...
          {items.length === 0 ? (
            ...
            <>
              ...
              <TablePagination
                ...
                sx={{
                  borderTop: '1px solid #F3F4F6', fontSize: '0.75rem',
          ...
          {selItem && (
            <Box sx={{
              mx: 2, mb: 2, p: 1.5, bgcolor: isIncident ? '#FFF8FF' : '#F8F9FF',
                ...
                {[
                  ...
                  <Box key={l}>
                    ...
                    <Typography noWrap sx={{
                      fontSize: '0.78rem', fontWeight: 600, color: '#111827',
                      fontFamily: l.includes('#') ? 'monospace' : 'inherit'
                    }}>{v}</Typography>
                  </Box>
                ))}
              </Box>
        ...
        <Paper elevation={0} sx={{ flex: 1, border: '1px solid #E5E7EB', borderRadius: '12px', overflow: 'hidden', minWidth: 0 }}>
          ...
          <Box sx={{
            ...
            <Stack direction="row" alignItems="center" spacing={1}>
              <Box sx={{
                width: 22, height: 22, borderRadius: '6px', bgcolor: BRAND,
                display: 'flex', alignItems: 'center', justifyContent: 'center'
              }}>
      ...
      <Paper elevation={3}
        ...
        <Stack direction="row" alignItems="center" spacing={1.5} flexWrap="wrap">
          ...
          {selItem && (
            <Box sx={{
              px: 1.5, py: 0.5, bgcolor: isIncident ? '#FDF0FB' : '#EEF0FB',
        ...
        <Stack direction="row" spacing={1.5}>
          {(selItem || selPerson) && (
            <Button variant="outlined" size="small"
              ...
              sx={{
                borderRadius: '8px', borderColor: '#D1D5DB', color: '#6B7280',
