  ...
  useEffect(() => {
    try {
      let trigger = async (id) => {
        let res = await triggerDuplicate(id);
        console.log(res);
        console.log(id);
      }
      tickets.map((e) => {
        if ((ticketIds.indexOf(e.id)) == -1) {
          trigger(e.id);
        }
        else {
          console.log(ticketIds);
        }
        setTicketIds(ticketIds.push(e.id))
      })
    } catch (error) {
      console.log(error);
    }
        ...
        <Stack direction="row" spacing={1.5} alignItems="center">
          ...
          <Box>
            ...
            <Typography variant="body2" sx={{ color: '#6B7280', mt: 0.2 }}>
              Manage and resolve tickets assigned to you — monitor SLA deadlines
            </Typography>
          </Box>
        </Stack>
        <Button
      ...
      <Paper elevation={0} sx={{ borderRadius: '14px', border: '1px solid #E5E7EB', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={8} align="center" sx={{ py: 6, color: '#9CA3AF' }}>Loading tickets…</TableCell></TableRow>
              ) : filtered.length === 0 ? (
                ...
                paginatedTickets.map(ticket => {
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{ cursor: 'pointer', ...(ticket.slaBreached && { backgroundColor: '#FFF6F8' }), '&:hover': { backgroundColor: '#F8F8FC' } }}
                    >
                      <TableCell>
