...
export default function L1Standalone() {
  ...
  const initPage = initTab === 'HISTORY' ? 'l1-history' : 'l1-pending';
  ...
  const handleSetPage = (pageId) => {
    ...
    if (approvalPages.includes(pageId)) {
  ...
  const isL2 = currentPage.startsWith('l2-');

  return (
    <Layout page={currentPage} setPage={handleSetPage} currentUser={currentUser}>
      {isL2
        ? <></>  // shouldn't happen in L1 standalone
