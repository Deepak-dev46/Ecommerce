...
import { authApi } from '../../api/authApi';
import { useAuth } from '../../context/AuthContext';
import { userApi } from '../../api/ourApi';
...
// REPLACE only the onSubmit function (everything else unchanged):
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // ── Enrich with allowedFeatures ───────────────────────────────────────
      let allowedFeatures = null;
      const effectiveRoles = response.effectiveRoles || response.roles || [];
 
      if (!effectiveRoles.includes('ADMIN') && response.token) {
        try {
      ...
      login({ ...response, allowedFeatures }, response.token);
      ...
      else if (roles.includes('ITSM_MANAGER'))      navigate('/itsm/overview');
      else if (roles.includes('SUPPORT_PERSONNEL')) navigate('/support/dashboard');
      else if (roles.includes('END_USER')) {
        const effective = response.effectiveRoles || [];
        if      (effective.includes('APPROVAL_MANAGER_L1') ||
                 effective.includes('APPROVAL_MANAGER_L2')) navigate('/approvals/queue');
        else if (effective.includes('RESOURCE_OWNER'))       navigate('/resource-owner/dashboard');
        else if (effective.includes('ITSM_MANAGER'))         navigate('/itsm/overview');
        else                                                  navigate('/user/dashboard');
      }
      else navigate('/user/dashboard');
