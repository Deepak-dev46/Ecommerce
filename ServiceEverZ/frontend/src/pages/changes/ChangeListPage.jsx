...
import { getAllChangePlans, getChangePlanById } from '../../api/changeApi';
...
const ALL_STATUSES = ['All', 'DRAFT', 'PENDING_APPROVAL', 'APPROVED', 'REJECTED', 'REVISION_REQUESTED', 'CANCELLED'];
const TAB_LABELS   = {
...
export default function ChangeListPage() {
  ...
  useEffect(() => {
    ...
    setFiltered(list);
    setPage(0);
  }, [changes, tab, search, priority, typeFilter]);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)', overflow: 'hidden' }}>
      ...
      <Box sx={{ p: 2, pb: 0, flexShrink: 0 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
          <Box>
            <Typography variant="h5" sx={{ fontWeight: 700, color: '#27235C' }}>Change Plans</Typography>
        ...
        <Stack direction="row" sx={{ borderBottom: '2px solid #E5E7EB', overflowX: 'auto', '&::-webkit-scrollbar': { display: 'none' } }}>
          {ALL_STATUSES.map((s) => {
            const count  = s === 'All' ? changes.length : changes.filter((c) => c.status === s).length;
            const active = tab === s;
            return (
              <Box key={s} onClick={() => { setTab(s); setPage(0); }}
                sx={{ px: 2, pb: 1, cursor: 'pointer', whiteSpace: 'nowrap', borderBottom: active ? '2px solid #27235C' : '2px solid transparent', mb: '-2px' }}>
