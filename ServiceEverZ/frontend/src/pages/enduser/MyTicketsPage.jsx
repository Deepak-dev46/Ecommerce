...
//   return (
//     <MuiChip label={priority} size="small"
//       sx={{ fontWeight: 600, fontSize: '0.72rem', height: 22,
...
//   const headers = ['Ticket #', 'Subject', 'Category', 'Sub-Category', 'Item', 'Status', 'Priority', 'Created At'];
//   const rows = tickets.map(t => [
//     t.ticketNumber || '',
//     `"${(t.subject || '').replace(/"/g, '""')}"`,
//     t.category || '', t.subCategory || '', t.item || '',
//     t.status || '', t.priority || '',
...
// const STATUS_TABS = ['ALL', 'DRAFT', 'SUBMITTED', 'OPEN', 'IN_PROGRESS', 'RESOLVED', 'CLOSED'];
...
//     setLoading(true);
//     try {
//       // Use ourApi ticketApi (goes through gateway to /api/tickets/user-tickets)
//       const arr = await ticketApi.getByUser(user.userId);
//       const sorted = (Array.isArray(arr) ? arr : [])
//         .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
//       setTickets(sorted);

//       // Fetch approval info for non-draft tickets to show L1/L2 names
//       const approvalMap = {};
//       await Promise.allSettled(
//         sorted.filter(t => t.status !== 'DRAFT').map(async t => {
//           // try {
//           //   const ap = await approvalApi.getByTicket(t.id);
//           //   if (ap) approvalMap[t.id] = ap;
//           // } catch {}
//           const ap = await approvalApi.getByTicket(t.id);

//           if (ap?.data) {
//           approvalMap[t.id] = ap.data;
//           } 
//         })
//       );
...
//     // Navigate to the service catalog with the draft ticket stored in state
...
//       return !q || (t.ticketNumber || '').toLowerCase().includes(q)
...
//                   {['Ticket No.', 'Subject', 'Category', 'L1 Approver', 'L2 Approver', 'Status', 'Priority', 'Created'].map(h => (
//                     <TableCell key={h} sx={{ fontWeight: 700, fontSize: '0.75rem', color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', py: 1.5 }}>
//                       {h}
//                     </TableCell>
//                   ))}
//                 </TableRow>
...
//               <TableBody>
//                 {paginated.map(t => {
//                   const ap = approvals[t.id];
//                   const isDraft = t.status === 'DRAFT';
//                   // console.log(t);

//                   return (
//                     <TableRow key={t.id}
//                       onClick={() => !isDraft && viewTicket(t.id)}
//                       sx={{
...
//                               sx={{ fontSize: '0.68rem', py: 0.25, px: 1, minWidth: 0, borderColor: '#9CA3AF', color: '#6B7280' }}>
//                               Edit
//                             </Button>
//                             <Button size="small" variant="contained"
//                               disabled={submitting === t.id}
//                               onClick={e => submitDraft(e, t.id)}
//                               sx={{ fontSize: '0.68rem', py: 0.25, px: 1, minWidth: 0, backgroundColor: '#97247E', '&:hover': { backgroundColor: '#7a1c65' } }}>
...
//                         <Typography sx={{ fontWeight: 600, fontSize: '0.83rem', color: '#111827' }} noWrap>{t.subject || '—'}</Typography>
//                         <Typography sx={{ fontSize: '0.72rem', color: '#6B7280' }} noWrap>{t.subCategory} {t.item ? `· ${t.item}` : ''}</Typography>
//                       </TableCell>
//                       <TableCell sx={{ fontSize: '0.8rem', color: '#374151', py: 1.5 }}>{t.category || '—'}</TableCell>
//                       <TableCell sx={{ fontSize: '0.8rem', color: '#374151', py: 1.5 }}>{ap?.l1ApproverName || '—'}</TableCell>
//                       <TableCell sx={{ fontSize: '0.8rem', color: '#374151', py: 1.5 }}>{ap?.l2ApproverName || '—'}</TableCell>
//                       <TableCell sx={{ py: 1.5 }}><StatusBadge status={t.status} /></TableCell>
//                       <TableCell sx={{ py: 1.5 }}><PriorityBadge priority={t.priority?.name ? t.priority.name() : t.priority} /></TableCell>
//                       <TableCell sx={{ fontSize: '0.78rem', color: '#6B7280', py: 1.5 }}>{formatDate(t.createdAt)}</TableCell>
//                     </TableRow>
//                   );
//                 })}
//               </TableBody>
