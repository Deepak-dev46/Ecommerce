...
public class TicketService {
	 ...
	 */
 	public TicketResponse assignTicket(Long id, Long assigneeId, String name, String assignedByName, boolean isManual) {
 		...
 		if (isManual) {
 			// MANUAL ASSIGN: ITSM Manager picked someone directly.
 			// Save assigneeId + set IN_PROGRESS immediately — no acknowledge needed.
 			...
 			// AUTO ACKNOWLEDGE: support person clicked Acknowledge.
 			// Only NOW save assigneeId + assigneeName to ticket table and move to IN_PROGRESS.
 			// Before this point assigneeId is NULL so ticket does NOT appear in My Tickets.
 			if (currentStatus == TicketStatus.OPEN) {
 				ticket.setAssigneeId(assigneeId);
 				ticket.setAssigneeName(name);
 				ticket.setStatus(TicketStatus.IN_PROGRESS);
 				Ticket saved = ticketRepository.save(ticket);
 				recordHistory(saved, TicketStatus.ASSIGNED,
 						"Ticket acknowledged and assigned to " + agentLabel, 0L, agentLabel);
 				recordHistory(saved, TicketStatus.IN_PROGRESS, "Status changed to In Progress", 0L, agentLabel);
 				return TicketResponse.from(saved, List.of(), List.of(), List.of(), List.of(), null);
 			}
 		}
 
 		// Fallback — ticket not in OPEN status. Return current state without changes.
 		return TicketResponse.from(ticket, List.of(), List.of(), List.of(), List.of(), null);
 	}
